...
existing_credits = pd.read_csv("credits.csv")

combined_movies = pd.concat([existing_movies, movies_df]).drop_duplicates(subset='movie_id')
combined_credits = pd.concat([existing_credits, credits_df]).drop_duplicates(subset='movie_id')

combined_movies.to_csv("movies.csv", index=False)
combined_credits.to_csv("credits.csv", index=False)
